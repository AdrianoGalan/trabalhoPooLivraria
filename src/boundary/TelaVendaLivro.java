...
public class TelaVendaLivro implements ControleTelas, EventHandler<ActionEvent> {
	...
	/**
	 * MÈtodo handle - acao aos botoes e eventos da tela venda de livro.
	 */
	@Override
	...
	public Pane render() {
		...
		// combobox cliente a√ß√£o teclado
		...
		hbLivro.getChildren().add(new Label("Pre√ßo:"));
	...
	/**
	 * MÈtodo que busca informacoes do cliente
	 */
	...
	/**
	 * MÈtodo que busca informaÁıes do livro
	 */
	...
	/**
	 * MÈtodo que carrega comboBox do cliente.
	 * 
	 * @param nome
	 */
	...
	/**
	 * MÈtodo que carrega a combobox do livro.
	 * 
	 * @param titulo
	 */
	...
	/**
	 * MÈtodo que adiciona um livro ma lista.
	 */
	private void adicionaItemLista() {
		...
		try {
			...
			if ((livro.getQtsEstoque() - qtdLivro) >= 0) {
				...
				Mensagens.erro("Quantidade", "Quantidade invalida", "N√£o tem livro no estoque");
			}
		} catch (Exception e) {
	...
	/**
	 * MÈtodo que remove um livro da lista.
	 */
	...
	/**
	 * MÈtodo que finaliza a venda dos livros.
	 */
	...
	/**
	 * MÈtodo que limpa campo do cliente.
	 */
